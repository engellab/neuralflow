...
def custom(x, w, lambdafunc=None):
    ...
    x : numpy array (N,), dtype=float
        Grid points in which the model will be evaluated. N is the number of
        grid points.
    w : numpy array (N,), dtype=float
        Weights used to evaluate integrals by the Gaussian quadrature.
    lambdafunc: function or None
        Either supply a function, or leave None if the model of the generated
        data is not known, in which case x and w are ignored.
...
def stepping(x, w,
    ...
    interp_x: numpy array (5,), dtype=float, or list
        ...
        sorted from the left to the right. Contains 5 values: the left
        boundary, the first maximum, the minimum, the second maximum and the
        right boundary. The default is [-1, -0.5, 0, 0.3, 1], which corresponds
        to the stepping potential used in M. Genkin, O. Hughes, T.A. Engel,
    ...
    interp_y: numpy array (5,), dtype=float, or list
        ...
        interp_x. The default is [0, 4, 0, 1, -1], which corresponds to the
        stepping potential used in M. Genkin, O. Hughes, T.A. Engel paper,
        Nat Commun 12, 5986 (2021).
    bc_left: list
        A list that contains two tuples that specify boundary conditions for
        the potential on the left boundary. The format is the same as in
        bc_type argument of scipy.interpolate.CubicSpline function. The default
        is [(1, 0), (1, 0)], which corresponds to a zero-derivative (Neumann)
        BCs for the potential.
    bc_right: list
        Same as bc_left, but for a right boundary. The default is
        [(1, 0), (1, 0)], which corresponds to a zero-derivative (Neumann) BCs
        for the potential.
    ...
    # Find additonal anchoring points on left and right boundaries to make the
    # potential around minima/maxima symmetric
    ...
    # Use three points for boundary splines, and two points for splines in the
    # bulk
    cs[0:2, :] = CubicSpline(xv_new[:3], yv_new[:3], bc_type=bc_left).c.T
    for i in range(1, xv.shape[0]-2):
        cs[i+1, :] = CubicSpline(xv[i:i+2], yv[i:i+2],
                                 bc_type=[(1, 0), (1, 0)]).c.T
...
def manual(x, w, interp_x=[-1, -0.3, 0, 0.3, 1],
           interp_y=[0, 10, 5, 10, 0],
           bc_left=[(1, 0), (1, 0)],
           bc_right=[(1, 0), (1, 0)],
           stationary_points=None):
    """Define arbitrary model by specifiying minima and maxima of the
    potential
    """
    ...
    i = 0
    while i < xv.shape[0]-1:
        if stationary_points is not None and stationary_points[i+1] == 0:
            cs[i:i+2, :] = CubicSpline(xv[i:i+3],
                                       yv[i:i+3], bc_type=[(1, 0), (1, 0)]).c.T
            i += 2
        else:
            cs[i, :] = CubicSpline(xv[i:i+2], yv[i:i+2],
                                   bc_type=[(1, 0), (1, 0)]).c.T
            i += 1
...
def double_well(x, w, xmax=0, xmin=0.3, depth=4.0, sig=0):
    ...
    depth : float
        depth of the potential wells (U_max - U_min) for the symmetric case
        (sig=0). The default is 4.
    sig : float
...
def mixture(x, w, theta=0.5, model1={'model': 'cos_square', 'params': {}},
            model2={'model': 'double_well', 'params': {}}):
    """
    ...
    model1 : dictionary.
        Dictionary that contains model name (one of the built-in models) and
    ...
    if not model1['model'] in peq_model_mixtures:
        raise ValueError(
            "peq_model type for model mixtures should be one of "
            f"{peq_model_mixtures.keys()}, {model1['model']} was given."
            )
    if not model2['model'] in peq_model_mixtures:
        raise ValueError(
            "peq_model type for model mixtures should be one of "
            f"{peq_model_mixtures.keys()}, {model2['model']} was given."
            )
...
peq_model_types_ = {
    'cos_square': cos_square,
    'cos_fourth_power': cos_fourth_power,
    'single_well': single_well,
    'double_well': double_well,
    'stepping': stepping,
    'linear_pot': linear_pot,
    'uniform': uniform,
    'mixture': mixture,
    'custom': custom,
    'manual': manual
